...
export default function CustomHeading({ title, subTitle, icon, className }: Props) {
  return (
    <Card className={cn("w-full bg-white flex items-center gap-3 p-3 rounded-xl border border-slate-200/80 shadow-xs", className)}>
      {icon && (
      ...
      <div>
        <h2 className="text-md sm:text-sm lg:text-md font-extrabold text-slate-900 leading-tight">
          {title}
        </h2>
        {subTitle && (
